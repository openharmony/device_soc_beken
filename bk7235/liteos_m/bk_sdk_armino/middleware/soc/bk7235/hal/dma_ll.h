...
{
    hw->config_group[id].req_mux.dest_req_mux = (dma_ll_dev_to_req_mux(req_mux)) & 0xf;
}

static inline void dma_ll_set_src_req_mux(dma_hw_t *hw, dma_id_t id, uint32_t req_mux)
{
    hw->config_group[id].req_mux.src_req_mux = (dma_ll_dev_to_req_mux(req_mux)) & 0xf;
}
...
{
    hw->config_group[id].ctrl.v &= 0xFFFF;
    hw->config_group[id].ctrl.v |= ((tran_len - 1) << 16);
}
