...
{
	return (hw->fifo_data.fifo_data_16 & 0xFFFF);
}

static inline uint16_t adc_ll_set_dc_offset(adc_hw_t *hw, uint32_t value)
{
	return hw->dc_offset_ctrl.dc_offset = (value & 0xFFF);
}

static inline uint16_t adc_ll_get_dc_offset(adc_hw_t *hw)
{
	return (hw->dc_offset_ctrl.dc_offset);
}

static inline uint16_t adc_ll_set_gain(adc_hw_t *hw, uint32_t value)
{
	return hw->gain_ctrl.gain = (value & 0x1FFFF);
}

static inline uint16_t adc_ll_get_gain(adc_hw_t *hw)
{
	return (hw->gain_ctrl.gain & 0x1FFFF);
}
